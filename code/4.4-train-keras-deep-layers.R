# creates  network of 4 stacked dense layers, trains it and saves it in hdf5 format in /models dir
...
model %>% 
  layer_embedding(input_dim = vocab_size, input_length = 150, output_dim = 256) %>%
...
history <- model %>% 
  fit( # this will take a while...
    ...
    epochs = 40, batch_size = nrow(keras_input)/5, 
...
model %>% 
  save_model_hdf5("./models/deep.h5")
